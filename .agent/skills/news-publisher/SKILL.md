...
1.  **Primary**: **Vertex AI Imagen 3.0** (Google Cloud).
    ...
    *   *Prompt*: Dynamic, brand-aligned prompts generated by Gemini.
2.  **Secondary**: **Vertex AI Imagen 2.0**.
3.  **Fallback**: **Pollinations AI** (External API).
...
*   **`news_bot/image_generator.py`**: Contains the prompt logic (Color/Emotion analysis) and the `_generate_with_vertex` / `_generate_with_pil` implementations.
